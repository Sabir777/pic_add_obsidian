...
# Передаю в этот файл названия всех папок содержащихся в текущей директории за исключением скрытых папок
find . -maxdepth 1 -type d -not -path '*/.*' >> all_dir.vim
...
find ../$1/. -type d -not -path '*/.*' >> all_dir.vim
...
printf "1) Выберите картинку, которую вы хотите включить в заметку, используя поиск по шаблону\n\
...
printf "--------------------------------------------------------------------------------------------\n" >> all_pic.vim
echo "--------------------------------------------------------------------------------------------" >> all_pic.vim
echo "Редактируемая заметка (здесь появятся ссылки на перемещаемые картинки):" >> all_pic.vim
echo "$NOTE" >> all_pic.vim
printf "--------------------------------------------------------------------------------------------\n" >> all_pic.vim
printf "После того как вы закончили работу с этим окном завершите работу vim с помощью ZZ \
